/*
 * master.cpp - Master node for DST_MST_SLV MIS Pipeline
 *
 * Runs on login node. Connects to slave processes on cn04/cn05 via TCP.
 * Slaves offload BFS + Clustering to DPU, do MIS update on CPU.
 * Uses preprocessed batch files and MIS file.
 *
...
int main(int argc, char *argv[]) {
    ...
    cout << "================================================================================" << endl;
    cout << " DST_MST_SLV MIS Pipeline - MASTER (DPU offload: BFS + Clustering)" << endl;
    cout << "================================================================================" << endl;
    ...
    /* Receive slave setup timings (5 doubles: recv_part, ghost_init, host_graph, connect_dpu, send_graph) */
    ...
    for (uint32_t u = 0; u < num_nodes; u++) {
        ...
        for (uint32_t v : adj[u]) {
            if (v > u && v < num_nodes && membership[v]) {
                membership[u] = 0;
                repair_removed++;
                break;
            }
    ...
    for (uint32_t u = 0; u < num_nodes; u++) {
        ...
        bool has_mis_nb = false;
        for (uint32_t v : adj[u]) {
            if (v < num_nodes && membership[v]) { has_mis_nb = true; break; }
        }
        if (!has_mis_nb) {
            bool conflict = false;
            for (uint32_t v : adj[u]) {
                if (v < num_nodes && membership[v]) { conflict = true; break; }
            }
    ...
    cout << endl;
    cout << "================================================================================" << endl;
    cout << "             DST_MST_SLV MIS PROFILING RESULTS" << endl;
    cout << "================================================================================" << endl;
