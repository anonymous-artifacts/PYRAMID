/*
 * slave.cpp - Slave node for distributed v5 (TCP-only, real hardware)
 *
 * Runs on cn04/cn05 via SLURM. Listens on a TCP port for master connection.
 * Connects to local DPU at 192.168.100.2:12345 for BFS + clustering.
 *
 * Usage: ./slave <listen_port> <dpu_host:dpu_port>
 *   e.g. ./slave 9000 192.168.100.2:12345
 */
...
/* ===========================================================================
 * TCP COORDINATION WITH MASTER (replaces MPI version)
 * =========================================================================== */

static void tcp_coordinate_with_master(
        int master_sock,
        const vector<uint32_t> &changed_local_ids,
        const vector<uint8_t> &membership,
        const GhostManager &ghost_mgr,
        OwnedMisResult &mis_result,
    ...
    if (nc > 0) {
        ...
        for (auto &c : corrections) {
            ...
            if (lid != UINT32_MAX) {
                mis_result.sync.membership_changes.push_back({lid, (uint32_t)c.new_membership});
            }
        }
    }
    ...
    if (do_refresh) {
        ...
        if (nr > 0) {
            ...
            for (auto &d : refresh) {
                ...
                if (lid != UINT32_MAX)
                    mis_result.sync.membership_changes.push_back({lid, d.new_val});
            }
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        if (first_batch) {
            ...
            /* Empty deltas for first batch */
            OwnedMisResult empty_result;
            tcp_coordinate_with_master(master_sock, {}, membership, ghost_mgr, empty_result,
                                        prime_wall_ms, prime_dpu_bfs_ms, prime_dpu_clust_ms,
                                        0, prime_send_ms, prime_recv_ms, round);
        ...
        /* TCP coordination with master */
        tcp_coordinate_with_master(master_sock, mis_result.changed_local_ids, membership,
                                    ghost_mgr, mis_result, wall_ms, dpu_bfs_ms, dpu_clust_ms,
                                    mis_ms, send_ms, recv_ms, round);
