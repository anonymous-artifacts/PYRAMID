...
bool verifyMIS(const Graph& graph, const vector<Node>& nodes, const string& label) {
    ...
    #pragma omp parallel for num_threads(n_threads) reduction(+:violations_independence, violations_maximality)
    for (int u = 0; u < graph.num_nodes; u++) {
        if (nodes[u].membership) {
            ...
            if (graph.adj_list[u].empty()) continue; // isolated node, skip
...
int main(int argc, char* argv[]) {
    ...
    for (int b = 0; b < (int)selected.size(); b++) {
        ...
        auto neighborhoods = computeBFSNeighborhoods(batch, graph, hop_limit);
        ...
        // Topology update happens per-edge alongside MIS updates (same as v1.cpp)
        ...
        bool valid = verifyMIS(graph, nodes, "Batch " + to_string(b));
