#!/bin/bash
# start_slaves.sh — Launch slave processes on cn04 and cn05 via SLURM
...
if [[ "$1" == "stop" ]]; then
    ...
    scancel --name=mis_dist_slave_cn04 2>/dev/null
    scancel --name=mis_dist_slave_cn05 2>/dev/null
    echo "Done."
    exit 0
fi
...
echo "Starting MIS Distributed slaves on cn04 and cn05 (port ${SLAVE_PORT})..."
