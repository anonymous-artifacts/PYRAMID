#!/bin/bash
###############################################################################
# start_infra.sh - Start/stop relay+DPU for FullRDS_MIS on real DPU ARM
#
# Usage:
...
DPU_SRC="${SCRIPT_DIR}/FullRDS_MIS_dpu.c"
DPU_USER="ubuntu"
...
case "${1:-start}" in
    deploy)
        echo "Deploying FullRDS_MIS_dpu.c to DPU ARM on ${NODE}..."
        srun -p mtech --nodelist="${NODE}" --ntasks=1 --time=00:05:00 \
            scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
            "${DPU_SRC}" ${DPU_USER}@${DPU_IP}:~/FullRDS_MIS_dpu.c
        echo "Compiling on DPU ARM..."
        srun -p mtech --nodelist="${NODE}" --ntasks=1 --time=00:05:00 \
            ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
            ${DPU_USER}@${DPU_IP} "gcc -O2 -o ~/FullRDS_MIS_dpu ~/FullRDS_MIS_dpu.c"
        echo "Deploy done."
        ;;

    stop)
        echo "Stopping relay..."
        scancel --nodelist="${NODE}" --name="mis_p12_relay" 2>/dev/null || true
        echo "Done."
        ;;
...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${DPU_USER}@${DPU_IP} 'pkill -x FullRDS_MIS_dpu 2>/dev/null; sleep 1; nohup ~/FullRDS_MIS_dpu ${DPU_PORT} > /tmp/mis_p12_dpu.log 2>&1 &'
sleep 2
echo '[DPU] MIS P12 server started on ${DPU_IP}:${DPU_PORT}'
# Run relay
${RELAY_BIN} ${RELAY_PORT} ${DPU_IP} ${DPU_PORT}
# Cleanup
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${DPU_USER}@${DPU_IP} 'pkill -x FullRDS_MIS_dpu 2>/dev/null' || true
"
