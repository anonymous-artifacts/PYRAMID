/*
 * PartialRDS_MIS_dpu.c - DPU Server: BFS Phase 1 Offload for MIS
 *
 * Design:
 *   - DPU does adjacency check + BFS 2-hop exploration (Phase 1)
 *   - Sends back affected areas for CPU-side clustering + MIS update
 *
 * Protocol (per batch):
 *   Host -> DPU:
 ...
 *     [num_edges * 4B] aff_counts
 *     [4B total_affected]
 *     [total_affected * 4B] affected node IDs (flat)
 *     [8B bfs_time_us]
 *
 * Compile: gcc -O2 -o PartialRDS_MIS_dpu PartialRDS_MIS_dpu.c
 * Run:     ./PartialRDS_MIS_dpu [port]
 */
...
static uint32_t *aff_counts_arr = NULL;
...
static int load_graph(uint32_t *offsets, uint32_t *sorted_nbrs, uint8_t *init_mem,
    ...
    if (!adj || !mem || !vis_ver || !aff_buf) {
        fprintf(stderr, "[MIS-DPU] FATAL: alloc failed for graph metadata\n");
        return -1;
    }
    ...
    if (ne > 0) {
        ...
        if (!nbr_pool) {
            fprintf(stderr, "[MIS-DPU] FATAL: alloc failed for neighbor pool (%.1f GB)\n",
    ...
    printf("[MIS-DPU] Memory: adj=%.0f MB, pool=%.0f MB, vis=%.0f MB, aff=%.0f MB\n",
...
static void ensure_batch_capacity(uint32_t num_edges) {
    ...
    aff_counts_arr = (uint32_t *)malloc(aff_batch_cap * sizeof(uint32_t));
    if (!aff_starts || !aff_counts_arr) {
        fprintf(stderr, "[MIS-DPU] FATAL: batch scratch alloc failed\n");
        aff_batch_cap = 0;
    }
}
...
/* ---- Send BFS results (NEW: replaces send_cluster_results) ---- */

static int send_bfs_results(int client_fd, uint32_t *is_ins_arr, uint32_t num_edges,
                             uint32_t total_affected, uint64_t bfs_us) {
    /* [num_edges] [is_ins * num_edges] [aff_counts * num_edges] [total_affected] [aff_buf] [bfs_us] */
    if (send_all(client_fd, &num_edges, 4) < 0) return -1;
    if (send_all(client_fd, is_ins_arr, num_edges * 4) < 0) return -1;
    if (send_all(client_fd, aff_counts_arr, num_edges * 4) < 0) return -1;
    if (send_all(client_fd, &total_affected, 4) < 0) return -1;
    if (total_affected > 0)
        if (send_all(client_fd, aff_buf, total_affected * 4) < 0) return -1;
...
int main(int argc, char *argv[]) {
    ...
    printf("[MIS-DPU] BFS-Only Server on port %d.\n", PORT);
    printf("[MIS-DPU] Clustering + MIS offloaded to host CPU. Press Ctrl+C to stop.\n\n");

    while (1) {
        printf("[MIS-DPU] Waiting for host connection...\n");
        client_fd = accept(server_fd, NULL, NULL);
        if (client_fd < 0) continue;
        printf("[MIS-DPU] Host connected.\n");

        /* Receive graph */
        uint32_t n, ne;
        if (recv_all(client_fd, &n, 4) < 0 || recv_all(client_fd, &ne, 4) < 0) goto done;

        printf("[MIS-DPU] Receiving graph: %u vertices, %u CSR edges (%.1f GB)...\n",
        ...
        if (!off || !init_mem || (ne > 0 && !nbrs)) {
            fprintf(stderr, "[MIS-DPU] FATAL: recv buffer alloc failed\n");
        ...
        printf("[MIS-DPU] Graph loaded. Ready for batches.\n");
        ...
        while (1) {
            ...
            if (recv_all(client_fd, &num_edges, 4) < 0) {
                printf("[MIS-DPU] Host disconnected.\n"); break;
            }
            if (num_edges == SHUTDOWN_SENTINEL) {
                printf("[MIS-DPU] Session complete.\n"); break;
            }
            ...
            for (uint32_t i = 0; i < num_edges; i++) {
                ...
                if (remain < 3) {
                    /* Not enough space for even src+dst — skip remaining edges */
                ...
                aff_counts_arr[i] = (uint32_t)cnt;
                aff_total += cnt;
                if (aff_total >= aff_buf_cap) {
            ...
            uint64_t bfs_us = (t1.tv_sec - t0.tv_sec) * 1000000ULL + (t1.tv_usec - t0.tv_usec);

            /* Step 4: Send BFS results */
            if (send_bfs_results(client_fd, is_ins_arr, num_edges, aff_total, bfs_us) < 0) {
                free(raw); free(is_ins_arr); break;
            }

            printf("[MIS-DPU] Batch %d: %u edges, bfs=%lu us, total_aff=%u\n",
                   batch_num, num_edges, (unsigned long)bfs_us, aff_total);
        ...
        close(client_fd);
        printf("[MIS-DPU] Ready for next connection.\n\n");
    }
