#!/bin/bash
# start_infra.sh — Launch NOSYNC slave + DPU on cn04 and cn05 via SLURM
...
if [[ "${1:-}" == "stop" ]]; then
    ...
    for node in "${NODES[@]}"; do
        scancel --nodelist="${node}" --name="mis_nosync_slave" 2>/dev/null || true
    done
    echo "Done."
    exit 0
fi
...
echo "Starting MIS DST_MST_SLV_NOSYNC slave + DPU (slave port ${SLAVE_PORT}, DPU ${DPU_ADDR})..."
echo ""
...
for node in "${NODES[@]}"; do
    ...
    sbatch --job-name="mis_nosync_slave" \
           --partition=mtech \
           --nodelist="${node}" \
           --ntasks=1 \
           --cpus-per-task=16 \
