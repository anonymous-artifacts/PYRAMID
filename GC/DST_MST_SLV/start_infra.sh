#!/bin/bash
# start_infra.sh — Launch GC slave + DPU on cn04 and cn05 via SLURM
...
if [[ "${1:-}" == "stop" ]]; then
    ...
    for node in "${NODES[@]}"; do
        scancel --nodelist="${node}" --name="gc_dst_slave" 2>/dev/null || true
    done
    echo "Done."
    exit 0
fi
...
echo "Starting GC DST_MST_SLV slave + DPU (slave port ${SLAVE_PORT}, DPU ${DPU_ADDR})..."
echo ""
