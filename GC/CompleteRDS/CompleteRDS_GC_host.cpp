// CompleteRDS_GC_host.cpp — Dynamic Graph Coloring with ALL phases offloaded to DPU
// Phase 1 (ProcessCE) + Phase 2 (CheckConflict) + Phase 3 (UpdateNeighbors): DPU via TCP
// Host: reads batches, classifies edges, sends to DPU, receives results, verifies
//
// No sync back-channel needed — DPU owns the graph entirely.
//
// Compile: g++ -O3 -fopenmp -std=c++17 CompleteRDS_GC_host.cpp -o CompleteRDS_GC_host
// Run:     ./CompleteRDS_GC_host <MTX> <Colors> <BatchesFolder> <NumBatches> <Threads> <DPU_Host> [DPU_Port]
...
static int connect_to_dpu(const char *host, int port, int timeout_sec = 5) {
    ...
    if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        ...
        cerr << "[Host] Make sure CompleteRDS_GC_dpu is running on the DPU device." << endl;
        close(sock); return -1;
    }
...
int main(int argc, char* argv[]) {
    ...
    int dpu_port = (argc > 7) ? stoi(argv[7]) : 5005;

    cout << "==================== CompleteRDS_GC ====================" << endl;
    ...
    for (int b = 0; b < (int)selected.size(); b++) {
        ...
        // ===== Pre-Phase: Read + Sort + Classify + Graph Update =====
        ...
        // ===== ALL PHASES ON DPU: ProcessCE + CheckConflict + UpdateNeighbors =====
        ...
        // Send: num_edges + edges only (no sync needed)
        ...
        // Receive full results from DPU
        uint32_t num_cc;
        if (recv_all(dpu_sock, &num_cc, 4) < 0) { cerr << "[Host] Recv failed" << endl; break; }
        if (num_cc > 0) {
            vector<ColorChange> cc(num_cc);
            if (recv_all(dpu_sock, cc.data(), num_cc * sizeof(ColorChange)) < 0) break;
            // Apply all color changes to local nodes (for verification)
            for (auto &c : cc) {
        ...
        // (always 0 — DPU does its own topology update)

        uint32_t iters_dpu;
        if (recv_all(dpu_sock, &iters_dpu, 4) < 0) break;

        uint64_t p1_us, p2_us, p3_us;
        if (recv_all(dpu_sock, &p1_us, 8) < 0) break;
        if (recv_all(dpu_sock, &p2_us, 8) < 0) break;
        if (recv_all(dpu_sock, &p3_us, 8) < 0) break;

        auto t_dpu_end = chrono::high_resolution_clock::now();
        bm.P123_Total = chrono::duration<float, milli>(t_dpu_end - t_dpu_start).count();
        bm.P123_DPU_P1 = (float)p1_us / 1000.0f;
        bm.P123_DPU_P2 = (float)p2_us / 1000.0f;
        bm.P123_DPU_P3 = (float)p3_us / 1000.0f;
        bm.P123_Transfer = bm.P123_Total - bm.P123_DPU_P1 - bm.P123_DPU_P2 - bm.P123_DPU_P3;
        bm.iterations = (int)iters_dpu;
        ...
        cout << fixed << setprecision(3)
             ...
             << " | P1: " << bm.P123_DPU_P1 << "ms"
             << " | P2: " << bm.P123_DPU_P2 << "ms"
             << " | P3: " << bm.P123_DPU_P3 << "ms"
    ...
    float avg_pre = 0, avg_p1 = 0, avg_p2 = 0, avg_p3 = 0;
    float avg_xfer = 0, avg_p123 = 0, avg_verify = 0, avg_total = 0;
    int tot_iters = 0;
    for (const auto& m : all_metrics) {
        avg_pre += m.PrePhase_CPU;
        avg_p1 += m.P123_DPU_P1;
        avg_p2 += m.P123_DPU_P2;
        avg_p3 += m.P123_DPU_P3;
    ...
    cout << "P123_DPU_P1              " << avg_p1 / cnt << endl;
    cout << "P123_DPU_P2              " << avg_p2 / cnt << endl;
    cout << "P123_DPU_P3              " << avg_p3 / cnt << endl;
    ...
    if (csv.is_open()) {
        if (!csv_exists) {
            csv << "Dataset,BatchRatio,BatchID,BatchFile,"
                << "PrePhase_CPU,P123_DPU_P1,P123_DPU_P2,P123_DPU_P3,"
        ...
        for (const auto& m : all_metrics) {
            csv << dataset_name << "," << batch_ratio << ","
                ...
                << m.PrePhase_CPU << "," << m.P123_DPU_P1 << ","
                << m.P123_DPU_P2 << "," << m.P123_DPU_P3 << ","
