/*
 * CompleteRDS_GC_dpu.c - DPU Server: Full Offload for Graph Coloring
 *
 * DPU handles ALL three phases:
 *   P1: ProcessCE (deletion optimization + insertion recoloring)
 *   P2: CheckConflict (resolve same-color neighbors)
 *   P3: UpdateNeighbors (propagate freed colors for chromatic reduction)
 *
 ...
 *
 *   Per batch DPU->Host:
 *     [4B num_color_changes]       all color changes from P1+P2+P3
 *     [num_cc * 12B]               ColorChange (node_id, old_color, new_color)
 *     [4B num_edge_muts]           always 0 (host does topology locally)
 *     [4B iters_dpu]               P2+P3 convergence iteration count
 *     [8B p1_us]                   ProcessCE time
 *     [8B p2_us]                   CheckConflict time
 *     [8B p3_us]                   UpdateNeighbors time
 *
 * Based on: FullRDS_GC_dpu.c + UpdateNeighbors added
 * Reference: final_set_gc_v2/CPU_DPU_P123_GC/CPU_DPU_P123_GC.cpp
 *
 * Compile: gcc -O2 -o CompleteRDS_GC_dpu CompleteRDS_GC_dpu.c
 * Run:     ./CompleteRDS_GC_dpu [port]
 */
...
static int load_graph(uint32_t *offsets, uint32_t *sorted_nbrs, int32_t *init_colors,
    ...
    if (!adj || !colors || !prev_colors) {
        fprintf(stderr, "[GC-P123-DPU] FATAL: alloc failed for graph metadata\n");
        return -1;
    }
    ...
    if (ne > 0) {
        ...
        if (!nbr_pool) {
            fprintf(stderr, "[GC-P123-DPU] FATAL: neighbor pool alloc failed\n");
            return -1;
        }
        nbr_pool_size = ne;
    ...
    printf("[GC-P123-DPU] Memory: adj=%.0f MB, pool=%.0f MB, colors=%.0f MB\n",
...
/* ---- UpdateNeighbors (Phase 3) ---- */

static void update_neighbors(uint32_t *aff, uint32_t num_aff,
                               uint32_t *next_aff, uint32_t *num_next,
                               ColorChange *cc_buf, uint32_t *num_cc) {
    uint32_t nn = 0;
    ...
    for (uint32_t i = 0; i < num_aff; i++) {
        ...
        for (uint32_t j = 0; j < adj[v].count; j++) {
            uint32_t u = adj[v].nbrs[j];
            if (freed_color >= colors[u]) continue; /* only pull down to lower color */

            /* Check if freed_color is still unused in u's neighborhood */
            int in_sc = 0;
            for (uint32_t k = 0; k < adj[u].count; k++) {
                if (colors[adj[u].nbrs[k]] == freed_color) { in_sc = 1; break; }
            }
            if (!in_sc) {
                ...
                next_aff[nn++] = u;
                cc_buf[*num_cc].node_id = u;
                cc_buf[*num_cc].old_color = old;
                cc_buf[*num_cc].new_color = freed_color;
                (*num_cc)++;
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("[GC-P123-DPU] Full Offload Server on port %d.\n", port);
    printf("[GC-P123-DPU] DPU handles: ProcessCE + CheckConflict + UpdateNeighbors.\n");
    printf("[GC-P123-DPU] CPU handles: batch read + topology update + verification.\n\n");

    while (1) {
        printf("[GC-P123-DPU] Waiting for host connection...\n");
        client_fd = accept(server_fd, NULL, NULL);
        if (client_fd < 0) continue;
        printf("[GC-P123-DPU] Host connected.\n");

        /* Receive graph */
        uint32_t n, ne;
        if (recv_all(client_fd, &n, 4) < 0 || recv_all(client_fd, &ne, 4) < 0) goto done;

        printf("[GC-P123-DPU] Receiving graph: %u vertices, %u CSR edges...\n", n, ne);
        ...
        if (!off || !init_colors || (ne > 0 && !nbrs)) {
            fprintf(stderr, "[GC-P123-DPU] FATAL: recv buffer alloc failed\n");
        ...
        printf("[GC-P123-DPU] Graph loaded. Ready for batches.\n");

        /* Scratch buffers (need extra capacity for P3 color changes) */
        uint32_t *aff_buf = (uint32_t *)malloc(n * sizeof(uint32_t));
        uint32_t *aff_buf2 = (uint32_t *)malloc(n * sizeof(uint32_t));
        ColorChange *cc_buf = (ColorChange *)malloc((size_t)n * sizeof(ColorChange));
        if (!aff_buf || !aff_buf2 || !cc_buf) {
            fprintf(stderr, "[GC-P123-DPU] FATAL: scratch alloc failed\n");
        ...
        while (1) {
            ...
            if (recv_all(client_fd, &num_edges, 4) < 0) {
                printf("[GC-P123-DPU] Host disconnected.\n"); break;
            }
            if (num_edges == SHUTDOWN_SENTINEL) {
                printf("[GC-P123-DPU] Session complete.\n"); break;
            }

            batch_num++;

            /* Step 1: Receive batch edges only (no sync needed) */
            ...
            if (num_edges == 0) {
                ...
                send_all(client_fd, &zero, 4);  /* 0 color changes */
                send_all(client_fd, &zero, 4);  /* 0 edge muts */
                send_all(client_fd, &zero, 4);  /* 0 iters */
                send_all(client_fd, &zero64, 8); /* p1_us */
                send_all(client_fd, &zero64, 8); /* p2_us */
                send_all(client_fd, &zero64, 8); /* p3_us */
                free(edges);
                continue;
            }

            /* Step 2: Apply topology update (GC: graph update BEFORE coloring) */
            for (uint32_t i = 0; i < num_edges; i++) {
            ...
            /* Step 3: ProcessCE (Phase 1, timed) */
            ...
            /* Step 4: CheckConflict (Phase 2, timed) */
            ...
            /* Step 5: UpdateNeighbors (Phase 3, timed) */
            int p3_iters = 0;

            while (num_aff > 0 && p3_iters < 50) {
                uint32_t num_next = 0;
                update_neighbors(aff_buf, num_aff, aff_buf2, &num_next, cc_buf, &num_cc);
                uint32_t *tmp = aff_buf; aff_buf = aff_buf2; aff_buf2 = tmp;
                num_aff = dedup_u32(aff_buf, num_next);
                p3_iters++;
            }
            ...
            uint64_t p3_us = (t3.tv_sec - t2.tv_sec) * 1000000ULL + (t3.tv_usec - t2.tv_usec);
            ...
            uint32_t total_iters = (uint32_t)(p2_iters + p3_iters);
            if (send_all(client_fd, &total_iters, 4) < 0) { free(edges); break; }

            if (send_all(client_fd, &p1_us, 8) < 0) { free(edges); break; }
            if (send_all(client_fd, &p2_us, 8) < 0) { free(edges); break; }
            if (send_all(client_fd, &p3_us, 8) < 0) { free(edges); break; }

            printf("[GC-P123-DPU] Batch %d: %u edges, p1=%lu us, p2=%lu us (i=%d), p3=%lu us (i=%d), cc=%u\n",
                   batch_num, num_edges,
                   (unsigned long)p1_us, (unsigned long)p2_us, p2_iters,
                   (unsigned long)p3_us, p3_iters, num_cc);
        ...
        close(client_fd);
        printf("[GC-P123-DPU] Ready for next connection.\n\n");
    }
