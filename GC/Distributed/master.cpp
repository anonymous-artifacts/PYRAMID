/*
 * master.cpp - Master node for Distributed CPU-Only Graph Coloring Pipeline
 *
 * Runs on login node. Connects to slave processes on cn04/cn05 via TCP.
 * Handles graph loading, initial coloring, METIS partitioning, batch distribution,
 * boundary color conflict resolution, and ghost refresh.
 *
 * No DPU — slaves run all 3 phases (ProcessCE + CheckConflict + UpdateNeighbors) on CPU.
 *
...
struct ColorResolutionResult {
    vector<DeltaEntry> corrections;  /* {node_id, new_color} */
    int iterations;
};
...
static ColorResolutionResult resolve_boundary_color_conflicts(
    ...
    /* Apply slave-reported color changes */
    ...
    /* Collect all differences from original */
    for (uint32_t u = 0; u < nv; u++) {
...
int main(int argc, char *argv[]) {
    ...
    cout << "================================================================================" << endl;
    cout << " Distributed CPU-Only GC Pipeline - MASTER" << endl;
    cout << "================================================================================" << endl;
    ...
    /* Load initial colors */
    auto t_col0 = chrono::high_resolution_clock::now();
    ...
    auto t_col1 = chrono::high_resolution_clock::now();
    double colors_load_ms = chrono::duration_cast<chrono::microseconds>(t_col1 - t_col0).count() / 1000.0;
    ...
    /* Create dummy membership for METIS partitioner (it expects uint8_t membership) */
    ...
    for (int p = 0; p < K; p++) {
        ...
        /* Send partition (uses MIS protocol but we'll send colors separately) */
    ...
    /* Receive setup timings (3 doubles) */
    ...
    vector<double> ss_p1_v(K, 0), ss_p2_v(K, 0), ss_p3_v(K, 0), ss_wall_v(K, 0);
    ...
    for (int round = 0; round < (int)selected.size(); round++) {
        ...
        /* Update master's adj with batch edges (topology update before coloring for GC) */
        for (auto &e : all_edges) {
        ...
        /* Receive boundary color deltas + timing from all slaves (4 doubles) */
        vector<vector<DeltaEntry>> all_deltas(K);
        vector<double> slave_p1(K), slave_p2(K), slave_p3(K), slave_wall(K);
        ...
        for (int p = 0; p < K; p++) {
            ...
            slave_p1[p] = recv_double(slave_socks[p]);
            slave_p2[p] = recv_double(slave_socks[p]);
            slave_p3[p] = recv_double(slave_socks[p]);
        }

        for (int p = 0; p < K; p++) {
            ss_p1_v[p] += slave_p1[p];
            ss_p2_v[p] += slave_p2[p];
        ...
        /* Send corrections to each slave (using DeltaEntry) */
        for (int p = 0; p < K; p++) {
        ...
        cout << "[Batch " << (round+1) << "/" << selected.size() << "] wall=" << fixed << setprecision(3) << round_ms << "ms"
             << " corrections=" << cr_result.corrections.size() << endl;
    }
    ...
    /* Receive final colors from each slave */
    for (int p = 0; p < K; p++) {
        auto final_colors = recv_vec<DeltaEntry>(slave_socks[p]);
        for (auto &d : final_colors)
    ...
    cout << endl;
    cout << "================================================================================" << endl;
    cout << "             DISTRIBUTED CPU-ONLY GC PROFILING RESULTS" << endl;
    cout << "================================================================================" << endl;
    ...
    if (csv.is_open()) {
        if (!csv_exists) {
            csv << "Dataset,BatchRatio,InitChromatic,FinalChromatic,Conflicts,"
                ...
                << "AvgP1_P0,AvgP2_P0,AvgP3_P0,AvgWall_P0,"
                << "AvgP1_P1,AvgP2_P1,AvgP3_P1,AvgWall_P1,"
        ...
        for (int p = 0; p < min(K, 2); p++) {
            if (batches_completed > 0) {
                csv << ss_p1_v[p]/batches_completed << ","
                    << ss_p2_v[p]/batches_completed << ","
                    << ss_p3_v[p]/batches_completed << ","
