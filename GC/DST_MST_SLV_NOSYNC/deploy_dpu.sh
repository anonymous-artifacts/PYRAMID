#!/bin/bash
# deploy_dpu.sh — Deploy and compile GC DPU kernel on ARM cores
# (Same DPU kernel as DST_MST_SLV — DPU logic is unchanged)

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
DPU_SRC="${SCRIPT_DIR}/dpu_kernel_gc.c"
DPU_USER="ubuntu"
...
if [[ ! -f "${DPU_SRC}" ]]; then
    echo "ERROR: dpu_kernel_gc.c not found at ${DPU_SRC}"
    exit 1
fi

echo "=== Deploying GC DST_MST_SLV_NOSYNC DPU kernel to ARM cores ==="
echo ""

for node in "${NODES[@]}"; do
    echo "--- ${node} ---"

    echo "  [1/3] Copying dpu_kernel_gc.c to ${node}'s DPU..."
