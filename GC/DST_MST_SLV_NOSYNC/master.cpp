/*
 * master.cpp - Master node for DST_MST_SLV_NOSYNC Graph Coloring Pipeline
 *
 * NO-SYNC variant: slaves do local GC updates (P3), master applies deltas
 * directly without boundary color conflict resolution or ghost refreshes.
 *
...
int main(int argc, char *argv[]) {
    ...
    cout << "================================================================================" << endl;
    cout << " DST_MST_SLV_NOSYNC GC Pipeline - MASTER (NO sync, NO ghost refresh)" << endl;
    cout << "================================================================================" << endl;
    ...
    /* Receive setup timings (5 doubles) */
    ...
    for (int round = 0; round < (int)selected.size(); round++) {
        ...
        for (int p = 0; p < K; p++) {
            ...
            if (num_deltas > 0) {
                vector<DeltaEntry> deltas(num_deltas);
                recv_all(slave_socks[p], deltas.data(), num_deltas * sizeof(DeltaEntry));
    ...
    /* Verify coloring — report as-is (no repair) */
    int conflicts = 0;
    ...
    cout << endl;
    cout << "================================================================================" << endl;
    cout << "             DST_MST_SLV_NOSYNC GC PROFILING RESULTS" << endl;
    cout << "================================================================================" << endl;
    ...
    cout << "Total deltas:       " << total_deltas_applied << endl;
